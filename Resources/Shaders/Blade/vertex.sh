...
vec3 conf(vec3 norm) {
	...
	return rotateNormal(norm, zN*3.14159/4.);
}
...
void main() {
	...
	vec3 norm = (vec4(gl_Normal,0.)*modelMatrix).xyz;

	vNormal = rotateNormal(norm, 3.*cDirection+((180. + 110.)/180.*3.14159));
	vNormal = conf(vNormal);
	...
	// RIGHT DARK
	vNormal = rotateNormal(norm, 4.*cDirection+((180. + 35.)/180.*3.14159));
	vNormal = conf(vNormal);
