...
void main()
{
	...
	vec2 uv = gl_TexCoord[0].xy - .5;//(gl_FragCoord.xy / iResolution) - .5;
	...
	float value = 0.21*color.r + 0.72*color.g + 0.07*color.b;

	gl_FragColor=vec4(color.rgb*(value) + (1.-value)*min(pow(abs(col), vec3(1.2)), 1.0), color.a);
}
