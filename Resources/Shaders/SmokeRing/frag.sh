...
void draw_obj(vec3 O, mat3 M, vec2 pos, int mode) {
	...
	vec3 Pm = P+.5*l*D,                		// .5: deepest point inside cloud. 
		 Nm = normalize(Pm/(R*R)),     		// its normal
	     Nn = normalize(P/R);
...
vec4 addCloud() {
	vec2 uv = (gl_FragCoord.xy / iResolution.y -vec2(.85,.5));
	float z = .2;
	
	vec3 O = vec3(-15.*cos(z),15.*cos(z),15.*sin(z));	// camera
...
void main()
{
	...
	//gl_FragColor = addCloud();
	gl_FragColor = vec4(gl_FragColor.rgb,v*gl_FragColor.a);
}
