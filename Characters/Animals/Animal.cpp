...
Animal :: Animal(float x, float y, float z) : Character(x,y,z) {
}

void Animal :: update(GraphicsOGL* gl, float deltaTime) {
...
void Animal :: draw(GraphicsOGL* gl, float deltaTime) {

	Character :: draw(gl, deltaTime);
}
sudo apt-get install libalut-devsudo apt-get install libalut-dev
void Animal :: updateControl(GraphicsOGL* gl, float deltaTime) {
	srand(time(NULL));
