...
public:
    ...
    Sound(AL al, String fileName) {
        valid = !(loadALData(al, fileName) == AL.AL_FALSE);
	}
    
	Sound(AL al, String fileName, float volume) {
        valid = !(loadALData(al, fileName) == AL.AL_FALSE);
        this.volume = volume;
	}
	
private: 
	int loadALData(AL al, String fileName) {
...
public: 
	void playSound(AL al, int[] src) {
        al.alSourcePlay(src[0]);
	}
	
	void playSound(AL al, int index) {
        al.alSourcePlay(source.get(index)[0]);
	}
	
	int[] playSound(AL al, double x, double y, double z, double vX, double vY, double vZ) {
		int[] src = new int[1];
	...
	void pauseSound(AL al, int index) {
        al.alSourcePause(source.get(index)[0]);
	}

	void stopSound(AL al, int index) {
